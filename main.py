...
# ---------------- MONITOR ZONE HITS & PREBREAK (DEBUG) ----------------
        ...
        try:
            ...
            for symbol, zones in list(state.get("zones", {}).items()):
                ...
                if df_latest is None or df_latest.empty:
                    logger.debug(f"{symbol} - no recent price data")
                    continue
                last_price = float(df_latest['close'].iloc[-1])
                logger.debug(f"{symbol} last_price={last_price:.6f}, {len(zones)} zones")
                
                for z in zones:
                    low, high = z['zone']
                    logger.debug(f"Checking zone {z['id']} low={low:.6f} high={high:.6f}")
                    
                    # HIT
                    if z.get("hit_at") is None and low <= last_price <= high:
                        ...
                        logger.info(f"Zone HIT: {symbol} price={last_price:.6f} zone={low:.6f}-{high:.6f}")
                    ...
                    # PREBREAK (approach)
                        ...
                        if last_price > high and (last_price - high)/high <= pct and not state.get("events_marker", {}).get(key):
                            ...
                            logger.info(f"Prebreak approaching from above: {symbol} last_price={last_price:.6f}")
                            df_chart = fetch_klines(symbol, z['tf'], limit=200)
                            img = build_chart_bytes(df_chart, symbol, z['tf'], zone=z['zone']) if df_chart is not None else None
                            send_telegram(txt, photo_bytes=img)
                            ...
                            logger.info(f"Prebreak approaching from below: {symbol} last_price={last_price:.6f}")
                            df_chart = fetch_klines(symbol, z['tf'], limit=200)
                            img = build_chart_bytes(df_chart, symbol, z['tf'], zone=z['zone']) if df_chart is not None else None
                            send_telegram(txt, photo_bytes=img)
                            state.setdefault("events_marker", {})[key] = now.isoformat()
            ...
            for sym in state.get("watchlist", []):
                try:
                    ...
                    logger.debug(f"Watchlist update {sym} price={lp:.6f}")
                    send_telegram(f"🔎 Watchlist update: {sym} price {lp:.6f}")
                except Exception:
                    logger.exception(f"Watchlist update error for {sym}")
        except Exception as e:
            logger.exception("monitor_zone_hits error: %s", e)
        time.sleep(10)  # тестовий швидший інтервал
